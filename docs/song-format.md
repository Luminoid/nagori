...
| `video` | object or null | `{ provider: "youtube", id, title, barTimes }` where `id` is the 11-character YouTube id and `barTimes[i]` is the second at which bar `i` starts in the video (one entry per bar, never decreasing). Null means no video: the page offers tab playback only and shows neither the video frame nor the source switch. |
